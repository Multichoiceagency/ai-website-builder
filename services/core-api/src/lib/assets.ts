 ...
 *
 * Duplicated, knowingly, from `packages/assets/src/document.ts`: the service
 * cannot yet depend on `@platform/assets` (adding the workspace dependency is a
 * `package.json` change outside this change's scope). The bodies below are
 * copied verbatim and must stay that way — a fingerprint computed here has to
 * equal one computed there, or a platform preset and the workspace asset a user
 * saved from it would never be recognised as the same arrangement. Collapse
 * into one import the moment the dependency lands.
 */
