...
const NAV = [
  ...
  { to: '/users', label: 'Registered users' },
...
<template>
  <div class="min-h-screen bg-sunken">
    <header class="border-b border-line bg-ink text-paper">
      <div class="mx-auto flex h-14 w-full max-w-6xl items-center gap-4 px-5">
        ...
        <nav class="hidden items-center gap-1 lg:flex" aria-label="Admin">
          <NuxtLink
            v-for="item in NAV"
            :key="item.to"
            :to="item.to"
        ...
        <div class="ml-auto flex items-center gap-2 text-[0.75rem] text-paper/70">
          <span class="hidden sm:inline">{{ session?.user.email }}</span>
          <a
            :href="config.public.dashboardUrl"
            class="rounded-md border border-white/25 px-2.5 py-1.5 no-underline hover:bg-white/10"
          >
            Customer dashboard
          </a>
          <button
            type="button"
            class="rounded-md border border-white/25 px-2.5 py-1.5 lg:hidden"
      ...
      <nav
        v-if="mobileOpen"
        id="admin-mobile-nav"
        class="border-t border-white/10 px-5 py-3 lg:hidden"
      ...
      >
        <div class="mx-auto flex max-w-6xl flex-col gap-1">
          <NuxtLink
            ...
            :key="item.to"
            :to="item.to"
            class="rounded-md px-2.5 py-2 text-[0.875rem] font-medium no-underline"
            :class="isActive(item.to) ? 'bg-white/15 text-paper' : 'text-paper/80'"
          >{{ item.label }}</NuxtLink>
        </div>
      </nav>
    </header>
