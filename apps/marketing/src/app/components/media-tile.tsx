...
export default function MediaTile({ d, cids, styles }: { d: MediaTileData; cids: string[]; styles: MediaTileStyles }) {
  return (
    <div data-cid={cids[0]} className={cn("w-67 h-[29.775rem] min-h-px flex relative float-left mr-5 rounded-[20px] overflow-hidden 2xl:w-82.5 2xl:h-[586.7px]", styles.className)} aria-hidden={d.ariahidden}>
      <div data-cid={cids[1]} className={cn("h-[13.175rem] block absolute bottom-0 inset-x-0 min-w-0 pt-25 pb-[0.9375rem] px-[0.9375rem]", styles.className2)} style={{ backgroundImage: "linear-gradient(var(--clr-4) 0%, var(--foreground) 100%)" }}>
      ...
      <div data-cid={cids[6]} className={cn("block", styles.className7)}>
        <picture data-cid={cids[7]} className={cn("inline", styles.className8)}>
          ...
          {" "}
          <img data-cid={cids[9]} className={cn("w-full h-119 block max-w-full overflow-clip object-cover aspect-[9/16] 2xl:h-[36.6875rem]", styles.className10)} data-component="image" src={d.imgSrc} />
          {" "}
        </picture>
      </div>
    </div>
