...
import { routerSource as bloomRouterSource, jsxDevSource as openthornJsxDevSource, dbSource as openthornDbSource } from './runtime-sources'
...
function toBase64(str: string): string {
  ...
  for (let i = 0; i < bytes.length; i += 3) {
    // noUncheckedIndexedAccess: de index is aantoonbaar geldig binnen deze lus,
    // maar dat weet de compiler niet. ?? 0 houdt het gedrag identiek aan het
    // origineel (ontbrekende bytes tellen als 0 en worden alsnog gepad met '=').
    const b1 = bytes[i] ?? 0
